...
int agent_sem, matches_sem, mutex_sem;  // Изменил имя переменной
...
int main() {
  ...
  if (agent_sem == -1 || matches_sem == -1 || mutex_sem == -1) {
    perror("semget failed");
    shmdt(shared);
    return 1;
  }

  cout << "Курильщик со спичками (PID: " << getpid() << ") начал работу" << endl;

  while (!stop_flag && !shared->stop_flag) {
    sem_wait(matches_sem);

    sem_wait(mutex_sem);
    if (shared->is_tobacco && shared->is_paper) {
      shared->is_tobacco = false;
      shared->is_paper = false;
      cout << "Курильщик со спичками взял табак и бумагу" << endl;
      sem_post(mutex_sem);

      cout << "Курильщик со спичками скручивает сигарету..." << endl;
      random_delay(500, 1000);
      cout << "Курильщик со спичками курит..." << endl;
  ...
  cout << "Курильщик со спичками (PID: " << getpid() << ") завершает работу" << endl;
  shmdt(shared);
  return 0;
}